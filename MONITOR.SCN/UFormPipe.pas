...
uses
  ...
  Dialogs, Buttons, ExtCtrls , UFrameGroup, SensorTypes, UScanner,
...
type
  TFormPipe = class(TForm)
    ...
    procedure miStartScannerClick(Sender: TObject);
  //***** IMonitorMaster
  public
  ...
  public
    ...
    procedure TimerProc;
    function CreateScanner(FromTime,ToTime:TDateTime):TScanner;
  //*****
  private
...
uses Clipbrd, DaySelect, DateTimeSelect, Main, UFormScanner;
...
begin
  ...
  if (Sender<>ActiveGroup) and (ActiveGroup<>nil)
  then ActiveGroup.Active:=False;
...
begin
  SetFocusedControl(ActiveGroup.ActiveGraph);
  ActiveGroup.ActiveGraph.FrameEnter(Self);
  ActiveGroup.FrameEnter(Self);
...
begin
//  ActiveControl:=nil;
  ActiveGroup.ActiveGraph.FrameExit(Self);
  ActiveGroup.FrameExit(Self);
end;
...
function TFormPipe.CreateScanner(FromTime, ToTime: TDateTime):TScanner;
var
  i:Integer;
  FS:TFormScanner;
begin
  ...
  Result.Pipe:=Self;
  for i:=0 to Groups.Count-1 do Group[i].AddToScanner(Result);
  Result.addGraph(nil); // done
  Result.State:=ssNotReady;
  Result.Info:=Caption;
  FS:=TFormScanner.Create(Self);
  FS.meStartTime.Text:=DateTimeToStr(FromTime);
  FS.meStopTime.Text:=DateTimeToStr(ToTime);
  FS.Caption:='Сканер: '+Caption;
  FS.OnIsShortCut:=IsShortCut;
  FS.OnCaptureKey:=FormKeyPress;
  Result.Form:=FS;
  FS.Show;
  FormScanner:=FS;
end;

procedure TFormPipe.miStartScannerClick(Sender: TObject);
begin
  if Scanners.Count>0 then exit;
  CreateScanner(
//(*
    StrToDate('2002-11-04')+StrToTime('07:30:00'),
    StrToDate('2002-11-05')
{*)
    StrToDate('2006-09-05')+StrToTime('04:00:00'),
    StrToDate('2006-09-06')
//}
  );
//  CreateScanner(StrToDate('2004-05-01'),StrToDate('2004-10-01'));
end;
