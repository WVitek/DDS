...
uses
  ...
  Dialogs, Buttons, ExtCtrls, UFrameGraph, SensorTypes, UScanner,
...
type
  ...
  public
    ...
    procedure AddToScanner(Scanner:TScanner);
...
begin
  ...
  FormMessage.Memo.Text:=
    Format(
      ...
      '  %.1f км до конца участка'#13#10+
      '  %.1f км НПП'#13#10,
      [Comment,KmX,Km2-Km1-KmX,Km1+KmX]
    )+
...
//    FG.Active:=True;
  ...
  Active:=True;
  TFormPipe(Parent).SetFocusedControl(FG);
end;
...
procedure TFrameGroup.AddToScanner(Scanner: TScanner);
var
  i:Integer;
begin
  for i:=0 to Graphs.Count-1 do Scanner.addGraph(Graph[i]);
end;

function TFrameGroup.Get_Active: Boolean;
begin
  Result:=Tag and 1<>0;
end;

procedure TFrameGroup.Set_Active(const Value: Boolean);
var
  i:Integer;
begin
  if Value xor (Tag and 1<>0) then begin
    if Value then begin
      ...
      ActiveGraph.Active:=True;
    end
    else begin
      Tag:=Tag and not 1;
      ActiveGraph.Active:=False;
    end;
    if not SpyMode then for i:=0 to Graphs.Count-1
      do TFrameGraph(Graphs[i]).View1ChangeViewState;
  end;
end;
