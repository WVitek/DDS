...
type
  ...
  TADLF_Liner=class(TADL_Filter)
    constructor Create(SrcData:TAnalogDataList; LineLen:Integer);
  protected
    FLineLen:Integer;
    procedure refreshItems(FromNdx,Count:Integer); override;
  end;
...
begin
  ...
  A:=SrcData.I;
  AL:=Alpha; InvAL:=1-AL;
  AL2:=Sqr(Sqr(Alpha)); InvAL2:=1-AL2;
  Val:=0; DVal:=0;
  ...
  for i:=0 to Capacity-1 do begin
    ...
    if ValidAD(AD) then begin
      if Cnt=0 then begin
        ...
        Val:=Val*AL+AD.Value*InvAL;
//        DVal:=DVal*AL2+(AD.Value-Val)*InvAL2;
      end;
      Inc(Cnt);
    end
    else Val:=Val*AL+(Val+DVal)*InvAL;
    if (Cnt<=MinCnt) or (Cnt=0) then FItems[i].Flags:=0
    else FItems[i].Value:=Val+DVal;
  end;
end;
...
constructor TADLF_Liner.Create(SrcData: TAnalogDataList;
  LineLen: Integer);
begin
  inherited Create(SrcData);
  FLineLen:=LineLen;
...
procedure TADLF_Liner.refreshItems(FromNdx, Count: Integer);
var
  i,LL:Integer;
  A:TADDynamicArray;
  K,B:Double;
begin
  ...
  A:=SrcData.I;
  LL:=FLineLen;
  for i:=LL to Capacity-LL-1 do begin
    if CalcKB(A,i-LL,i+LL,K,B)>=LL then FItems[i].Value:=K*i+B;
  end;
end;
