object FormMain: TFormMain
  Left = 310
  Top = 209
  AutoScroll = False
  BorderIcons = [biSystemMenu, biMaximize]
  Caption = 'MultiLoader7188 full duplex'
  ClientHeight = 423
  ClientWidth = 632
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  ShowHint = True
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 13
  object TreeView: TTreeView
    Left = 0
    Top = 0
    Width = 113
    Height = 423
    Align = alLeft
    Indent = 19
    ReadOnly = True
    TabOrder = 0
    OnChange = TreeViewChange
  end
  object AppExt: TDdhAppExt
    Icon.Data = {
      }
    Title = 'MultiLoader7188'
    HintShortPause = 0
    TrayIconActive = True
    TrayIcon.Data = {
      }
    TrayHint = 'Tip'
    TrayPopup = PopupMenu
    OnTrayDefault = AppExtTrayDefault
    OnLBtnDown = AppExtTrayDefault
    Left = 16
    Top = 16
  end
  object PopupMenu: TPopupMenu
    Left = 16
    Top = 64
    object pmiShowHide: TMenuItem
      Bitmap.Data = {
        
        EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE}
      Caption = 'Показать / Спрятать'
      Default = True
      OnClick = AppExtTrayDefault
    end
    object N2: TMenuItem
      Caption = '-'
    end
    object pmiAbout: TMenuItem
      Bitmap.Data = {
        
        EE0EE0EEEEEEEEEEEE0EE00000000000000EEEEEEEEEEEEEEEEE}
      Caption = 'О программе ...'
      OnClick = pmiAboutClick
    end
    object pmiClose: TMenuItem
      Bitmap.Data = {
        
        0E0EE0EEEEEEEEEEEE0EE00000000000000EEEEEEEEEEEEEEEEE}
      Caption = 'Закрыть'
      OnClick = pmiCloseClick
    end
  end
  object NMUDP: TNMUDP
    RemotePort = 0
    LocalPort = 0
    ReportLevel = 1
    OnInvalidHost = NMUDPInvalidHost
    OnBufferInvalid = NMUDPBufferInvalid
    Left = 16
    Top = 112
  end
  object Timer: TTimer
    OnTimer = TimerTimer
    Left = 16
    Top = 160
  end
  object Modem: TModem
    DeviceName = 'Com2'
    MonitorEvents = [evBreak, evCts, evError, evRing, evRlsd, evRxChar, evRxFlag, evTxEmpty]
    Options = []
    FlowControl = fcNone
    OnConnFailed = ModemConnFailed
    OnConnect = ModemConnect
    OnDisconnect = ModemDisconnect
    OnModemRxChar = ModemModemRxChar
    OnResponse = ModemResponse
    Left = 16
    Top = 208
  end
  object TimerProcessIO: TTimer
    Interval = 20
    OnTimer = ProcessIO
    Left = 16
    Top = 256
  end
end
