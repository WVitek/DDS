...
function GetCRC16(var Data; DataLen:Integer):Word;
function SR_WStr(S:TStream):WideString;
procedure SW_WStr(S:TStream;W:WideString);
function SR_Int(S:TStream):Integer;
procedure SW_Int(S:TStream; const I:Integer);
function SR_Dbl(S:TStream):Double;
procedure SW_Dbl(S:TStream; const D:Double);
...
begin
  ...
  while (DataLen>0) do begin
    Index:=CRCHi xor PB^ ; // calculate the CRC16
    CRCHi:=CRCLo xor CRC16Hi[Index];
    CRCLo:=CRC16Lo[Index];
    Inc(PB);
    Dec(DataLen);
  end;
  Result:=(Integer(CRCHi) shl 8) or CRCLo;
end;
