//******************************************************************************
//                      VARIAN ASYNC32 COMPONENT v1.48
...
//Due to the massive response on our components we can't garantuee that we
//have the time to write a reply on your mail. We therefore urge you not
//to sent us any questions regarding standard programming with Delphi but
//only to sent us bug reports or suggestions.
...
begin
  ...
  while (not Terminated) do begin
    ...
    then WaitEventResult := (FEvent.WaitFor(INFINITE) = wrSignaled);
...
var
  ...
  CCE_Ok:Boolean;
begin
  CCE_Ok:=ClearCommError(FHandle, Errors, @ComStat);
  if (Status and EV_RXCHAR > 0) or (Status and EV_RLSD > 0) then begin
    if CCE_Ok and (ComStat.cbInQue > 0) and assigned(FOnRxChar)
    then FOnRxChar(self, ComStat.cbInQue);
  end;
  ...
  if Status and EV_RLSD > 0 then begin
  ...
  if Status and EV_ERR > 0 then
    if CCE_Ok and assigned(FOnError) then FOnError(self, Errors);
