...
constructor TADLF_Krivizna.Create(SrcData: TAnalogDataList; Alpha: Double);
begin
  inherited Create(SrcData);
  Self.Alpha:=Alpha;
end;
...
begin
  ...
  A:=SrcData.I; InvAL:=1-Alpha; AL:=Alpha;
  Avg:=0; Val:=0; Cnt:=0;
  MinCnt:=Round(LnPrecision/Ln(Alpha));
  for i:=0 to Capacity-1 do begin
    AD:=A[i];
    if ValidAD(AD) then begin
      if Cnt=0 then Avg:=AD.Value
      else Avg:=Avg*AL+AD.Value*InvAL;
      Val:=AD.Value-Avg;
      Inc(Cnt);
    end;
    if (i<=MinCnt) or (Cnt=0) then FItems[i].Flags:=0
...
begin
  ...
  Dst:=FItems;
  i:=0;
  while (i<Capacity) and not ValidAD(Src[i]) do Inc(i);
  if (i<Capacity) then Prev:=Src[i].Value else Prev:=0;
  Dst[0].Value:=SignedZero;
  for i:=1 to Capacity-1 do begin
    if ValidAD(Src[i]) then begin
      Tmp.Value:=Src[i].Value-Prev;
      Prev:=Src[i].Value;
      if Tmp.Value=0 then Tmp.Value:=SignedZero;
    end
    else Tmp.Flags:=0;
